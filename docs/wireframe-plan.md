...
|------|---------|--------|-----|--------|
| 01_Home | 홈 | `/` | W-01, W-01b(챗 열림), W-01c(챗 대화중) | M-01, M-01b(메뉴 열림), M-01c(챗 전체화면) |
...
web 15개 + mobile 16개 = 31개 프레임. 각 화면의 헤더는 해당 탭이 활성(검정 + 밑줄) 상태로 override돼 있다.

### ASK GATS 챗 패널

실측: 데스크톱은 480×640 패널이 우하단(우 28px, 하 24px)에 뜨고 플로팅 필은 사라진다. 모바일은 전체화면(390×844)으로 덮는다.

| 상태 | 구성 |
|------|------|
| 제안 (W-01b, M-01c) | 헤더(폰 아이콘 + GATS LAB AI + 초록 닷 + 닫기) / AI ASSISTANT 라벨 + 안내문 / 제안 카드 5종 — PROJECT·TECH·CAREER·INFRA·CONTACT / 하단 입력바 + 원형 전송 버튼 |
| 대화중 (W-01c) | 헤더에 대화 지우기 아이콘 추가 / 사용자 메시지는 검정 채움 말풍선 우측 정렬 / 답변은 모노 `ASSISTANT` 라벨 아래 말풍선 없는 본문 / 입력바 동일 |

제안 카드의 질문은 실제 노출 문구를 그대로 썼다 (예: `왜 Ollama를 선택했나요?`, `Docker 서비스 구성이 어떻게 되나요?`). 대화 상태의 답변도 로컬 ollama가 실제로 생성한 응답이다.
...
| CMP_챗패널_제안_web / _대화_web | 챗 패널 480×640 두 상태 |
| CMP_챗패널_mobile | 챗 패널 전체화면 390×844 |
